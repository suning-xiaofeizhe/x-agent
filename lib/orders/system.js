...
function getNodeCount(callback) {
  utils.execCommandAsync('get_node_processes.js').then(data => {
...
exports.run = function (callback) {
  ...
  getFreeMemory(function (err, freemem) {
    ...
    getLoadAvg(function (err, load) {
      ...
      getCpuUsage(function (err, used_cpu) {
        if (err) {
          return callback(err);
        }
        result.metrics.used_cpu = used_cpu;
        getDiskUsage(function (err, disks) {
          if (err) {
            return callback(err);
          }
          result.metrics.disks = disks;
          getNodeCount(function (err, nodes) {
            if (err) {
              return callback(err);
            }
            result.metrics.node_count = nodes.length;
